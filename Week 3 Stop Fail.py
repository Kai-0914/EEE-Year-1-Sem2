...
Kp = 0.4   # Proportional gain (Start with 0.5)
Ki = Kp * 0.001  # Integral gain (Start small)
...
def shapes(frame):
    ...
    upper_black = np.array([180, 50, 80])   # allow a bit of S/V for dark gray
    ...
    if   v == 3: shape = "triangle"
        ...
        if circ > 0.8:
            ...
            hull = cv2.convexHull(best, returnPoints=False)
            defects = cv2.convexityDefects(best, hull)
            if defects is not None:
                significant_defects = 0
                for i in range(defects.shape[0]):
                    s, e, f, d = defects[i, 0]
                    depth = d / 256.0  # Scale factor from OpenCV
                    if depth > 10:  # Adjust threshold based on testing
                        significant_defects += 1
                if significant_defects == 1 and 0.6 <= circ < 0.8:
                    shape = "Pacman"
...
try:
    while True:
        ...
        symbol, current_shape = shapes(symbol_raw.copy())
        
        current_time = time.time()
        
        if current_shape is not None and not is_stopped and (current_time - last_shape_time) >= SHAPE_COOLDOWN:
            is_stopped = True
            stop_start_time = current_time
            print(f"Detected {current_shape}. Stopping for 3 seconds.")
        ...
        if is_stopped:
            ...
            if current_time - stop_start_time < 3:
                continue
                
            is_stopped = False
            last_shape_time = current_time 
            print("Resuming movement. Cooldown started.")
